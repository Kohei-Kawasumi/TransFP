...
from uap_attack.early_stopping_patch import EarlyStopping
...
def get_model(name):
    ...
    if name == 'yolov3':
        ...
        # assuming similar structure and import style
        model = DetectBackend('yolov6s.pt').eval()
    elif name == 'yolov7':
        from local_yolos.yolov7.models.experimental import attempt_load
        model = attempt_load('yolov7.pt', map_location=device).eval()
    elif name == 'yolov8':
        # adjust import and loading for yolov8
        from ultralytics import YOLO
        model = YOLO('yolov8l.pt')  # load a pretrained model (recommended for training)
        model.eval()
    elif name == 'yolov9':
        # adjust import and loading for yolov9
        from yolov9.models.common import DetectMultiBackend
        model = DetectMultiBackend('yolov9-s-converted.pt', device).eval()
    elif name == 'detr':
...
class GEN_UAP:
    def __init__(self, patch_folder, train_loader, val_loader, epsilon=0.1, iter_eps=0.05, penalty_regularizer=0,
        ...
        if 6 in models_vers:
            self.models.append(get_model('yolov6'))
        if 7 in models_vers:
            self.models.append(get_model('yolov7'))
        if 8 in models_vers:
            self.models.append(get_model('yolov8'))
        if 9 in models_vers:
            self.models.append(get_model('yolov9'))
        if 10 in models_vers:
            self.models.append(get_model('detr'))
        else:
            print('no model')
        ...
        self.full_patch_folder = "uap_train_bdd/" + patch_folder + "/"
        Path(self.full_patch_folder).mkdir(parents=True, exist_ok=True)

        self.current_dir = "experiments_bdd/" + patch_folder
        self.create_folders()
    ...
    def evaluate_loss(self, loader, adv_patch):
        ...
        for (img_batch, lab_batch, _) in loader:
            ...
            with torch.no_grad():
                ...
                if r==3:
                        output_clean = output_clean.unsqueeze(0)
                ...
                if not torch.isnan(iou):
                    batch_loss += (iou.item() * ( self.lambda_1))
                    orig_classification_loss.append(iou.item() * (self.lambda_1))
    ...
    def max_objects(self, output_patch, conf_thres=0.5, target_class=2):
    ...
    def loss_function_gradient(self, applied_patch, init_images, batch_label, penalty_term, adv_patch):
        ...
        if not torch.isnan(iou_loss):
            loss += (iou_loss * (self.lambda_1))
            self.current_orig_classification_loss += (( self.lambda_1) * iou_loss.item())
